...
private:
	struct binaryNode {
		...
		int num;
		binaryNode(const elemType &x, binaryNode *lt = NULL, binaryNode *rt = NULL, int n=1):data(x), left(lt), right(rt), num(n) {}
	};
...
elemType binarySearchTree<elemType>::find_with_index(int i, const elemType &notfind) {
	...
	while(!s.isEmpty()) {
		...
		if (++current.TimesPop == 2) {
			if (times <= i and times >= i - current.node->num + 1) return current.node->data;
			times += current.node->num;
...
int main() {
	int s[] = {6,4,2,8,45,34,1,3,23,89,4,5,7,32,21,45,543,123,324,234,34,78,454,34};
